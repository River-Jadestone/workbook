...
// [신규] 웹 앱에 필요한 초기 데이터를 한 번에 묶어서 전달하는 함수입니다.
function getInitialData() {
  try {
    ...
    // 오류 발생 시, 클라이언트에서 오류를 처리할 수 있도록 오류 객체를 반환합니다.
...
function getWorkbooks() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('워크북');
  if (!sheet) throw new Error("'워크북' 시트를 찾을 수 없습니다.");
  const data = sheet.getDataRange().getValues();
  const headers = data.shift();
  
  const idIndex = headers.indexOf('워크북ID');
  const toolIndex = headers.indexOf('교구');
  const nameIndex = headers.indexOf('워크북이름');
  const descIndex = headers.indexOf('워크북설명');
  const typeIndex = headers.indexOf('워크북타입');
  const contentIndex = headers.indexOf('콘텐츠');
  const enabledIndex = headers.indexOf('사용여부');

  if ([idIndex, toolIndex, nameIndex, descIndex, typeIndex, contentIndex, enabledIndex].includes(-1)) {
      throw new Error("'워크북' 시트의 머리글(헤더)이 올바르지 않습니다.");
  }
...
function saveRecord(recordData) {
  try {
    ...
    const recordSheet = ss.getSheetByName('기록');
    if (!recordSheet) throw new Error("'기록' 시트를 찾을 수 없습니다.");

    const workbookSheet = ss.getSheetByName('워크북');
    const workbookData = workbookSheet.getDataRange().getValues();
    const workbookHeaders = workbookData.shift();
    const idIndex = workbookHeaders.indexOf('워크북ID');
    const contentIndex = workbookHeaders.indexOf('콘텐츠');

    let workbookContentString = workbookData.find(row => row[idIndex] === recordData.workbookId)?.[contentIndex];
    if (!workbookContentString) throw new Error(`워크북 ID '${recordData.workbookId}'를 찾을 수 없습니다.`);
    ...
    if (workbookContent.type === 'bricq-hybrid') {
        ...
        const formattedQuizzes = quizQuestions.map((q, index) => {
            return `[질문] ${q.question}\n➡️ [답변] ${quizAnswers[index] || '(답변 없음)'}`;
        }).join('\n\n');
        
        formattedThoughts = formattedQuizzes;

        if (workbookContent.spikeExtension && recordData.activityContent.spikeIdea) {
            const spikeIdeaTitle = workbookContent.spikeExtension.ideaPrompt || '나만의 아이디어';
            const spikeIdeaAnswer = recordData.activityContent.spikeIdea;
            formattedThoughts += `\n\n[코딩 아이디어] ${spikeIdeaTitle}\n➡️ [답변] ${spikeIdeaAnswer}`;
        }
        ...
        formattedThoughts = originalThoughts.map((question, index) => {
            return `[질문] ${question}\n➡️ [답변] ${studentAnswers[index] || '(답변 없음)'}`;
        }).join('\n\n');
    }
    ...
    const taskCol = headers.indexOf('활동내용(체크리스트)') + 1;
    const thoughtCol = headers.indexOf('생각나누기') + 1;
    ...
    return "성공적으로 저장되었습니다.";
  } catch (e) {
    console.error("saveRecord 오류: " + e.message);
    return "저장에 실패했습니다: " + e.message;
  }
}